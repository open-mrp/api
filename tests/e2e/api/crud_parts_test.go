...
func TestParts_ListSearchRanksExactSKUBeforeSubstring(t *testing.T) {
	t.Parallel()
	exactSKU := uniqueName("e2e-rank-pt-ex")
	...
	t.Cleanup(func() { apiClient.Delete(partsPath + "/" + longID) })
	...
	t.Cleanup(func() { apiClient.Delete(partsPath + "/" + exactID) })

	list, _, err := apiClient.GetList(partsPath, url.Values{"q": {exactSKU}, "include": {"item"}, "limit": {"10"}})
	require.NoError(t, err)
	require.NotEmpty(t, list.Data, "search should return both parts")
