// Package resourceloaders holds the gRPC-backed loaders used by the
// api-gateway resourcekit include resolver. Each loader is a plain function
// that takes a slice of IDs and returns a map[id]*apiresource (typed as any
// to fit resourcekit's signature). Loaders build clean apiresource values
// and stash any FK metadata the SubField closures will need into the
// request-scoped resourcekit.LoadMeta side-table.
//
// The core client is set once at process startup via SetCoreClient — the
// loaders cannot be invoked before that.
...
// coreClient is the primary CoreService gRPC client used by every loader
// whose RPC lives on CoreService (carrier, account, priority, etc.). Set by
// SetCoreClient during api-gateway startup before any HTTP traffic is served.
var coreClient pb.CoreServiceClient

// coreSalesClient is the CoreSalesService client (different proto service)
// used by loaders whose RPCs live in core_sales.proto (sales-order-status,
// sales-order, order-discount, etc.). Set together with coreClient via
// SetCoreClient(s).
var coreSalesClient pb.CoreSalesServiceClient

// agentClient is the AgentService client used by loaders whose RPCs live in
// agent.proto (agent-memory, agent-run, agent-alert, agent-tool, etc.). Set
// at startup alongside coreClient.
var agentClient agentpb.AgentServiceClient

// SetCoreClient is called once from the api-gateway wiring code with the
// shared CoreServiceClient wrapper. Calling it more than once overwrites the
// previous value — useful for tests that swap in mocks.
...
// fulfillmentClient is the CoreFulfillmentService client used by loaders
// whose RPCs live in core_fulfillment.proto (machine, etc.). Set at startup.
...
// auth.proto (api-key, etc.). Set at startup alongside coreClient.
...
// loggingClient is the LoggingService client used by loaders whose RPCs live
// in platform.proto (request-log, etc.). Set at startup.
