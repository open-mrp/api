// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: inventory_change_log.sql

package sqlc

import (
	"context"
	"database/sql"
	"strings"
	"time"
)

const getInventoryChangeLog = `-- name: GetInventoryChangeLog :one
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    i.created_at AS item_created_at,
    i.updated_at AS item_updated_at,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    u.ratio_numerator AS quantity_unit_ratio_numerator,
    u.ratio_denominator AS quantity_unit_ratio_denominator,
    u.offset_numerator AS quantity_unit_offset_numerator,
    u.offset_denominator AS quantity_unit_offset_denominator,
    u.created_at AS quantity_unit_created_at,
    u.updated_at AS quantity_unit_updated_at,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    ss.created_at AS scanning_station_created_at,
    ss.updated_at AS scanning_station_updated_at,
    icl.responsible_user_id,
    usr.name AS responsible_user_name,
    usr.created_at AS responsible_user_created_at,
    usr.updated_at AS responsible_user_updated_at
FROM inventory_change_log icl
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
WHERE icl.id = ?
AND icl.account_id = ?
`

type GetInventoryChangeLogParams struct {
	ID        string
	AccountID string
}

type GetInventoryChangeLogRow struct {
	ID                            string
	ActionTypeCode                string
	AccountID                     string
	CreatedAt                     time.Time
	UpdatedAt                     time.Time
	ItemID                        string
	ItemSku                       string
	ItemTypeCode                  string
	ItemCreatedAt                 time.Time
	ItemUpdatedAt                 time.Time
	QuantityID                    string
	QuantityValue                 string
	QuantityUnitID                string
	QuantityUnitName              string
	QuantityUnitAbbreviation      string
	QuantityUnitType              string
	QuantityUnitRatioNumerator    string
	QuantityUnitRatioDenominator  string
	QuantityUnitOffsetNumerator   string
	QuantityUnitOffsetDenominator string
	QuantityUnitCreatedAt         time.Time
	QuantityUnitUpdatedAt         time.Time
	ScanningStationID             sql.NullString
	ScanningStationName           sql.NullString
	ScanningStationType           sql.NullString
	ScanningStationCreatedAt      sql.NullTime
	ScanningStationUpdatedAt      sql.NullTime
	ResponsibleUserID             sql.NullString
	ResponsibleUserName           sql.NullString
	ResponsibleUserCreatedAt      sql.NullTime
	ResponsibleUserUpdatedAt      sql.NullTime
}

func (q *Queries) GetInventoryChangeLog(ctx context.Context, arg GetInventoryChangeLogParams) (GetInventoryChangeLogRow, error) {
	row := q.db.QueryRowContext(ctx, getInventoryChangeLog, arg.ID, arg.AccountID)
	var i GetInventoryChangeLogRow
	err := row.Scan(
		&i.ID,
		&i.ActionTypeCode,
		&i.AccountID,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.ItemID,
		&i.ItemSku,
		&i.ItemTypeCode,
		&i.ItemCreatedAt,
		&i.ItemUpdatedAt,
		&i.QuantityID,
		&i.QuantityValue,
		&i.QuantityUnitID,
		&i.QuantityUnitName,
		&i.QuantityUnitAbbreviation,
		&i.QuantityUnitType,
		&i.QuantityUnitRatioNumerator,
		&i.QuantityUnitRatioDenominator,
		&i.QuantityUnitOffsetNumerator,
		&i.QuantityUnitOffsetDenominator,
		&i.QuantityUnitCreatedAt,
		&i.QuantityUnitUpdatedAt,
		&i.ScanningStationID,
		&i.ScanningStationName,
		&i.ScanningStationType,
		&i.ScanningStationCreatedAt,
		&i.ScanningStationUpdatedAt,
		&i.ResponsibleUserID,
		&i.ResponsibleUserName,
		&i.ResponsibleUserCreatedAt,
		&i.ResponsibleUserUpdatedAt,
	)
	return i, err
}

const listAllInventoryChangeLogs = `-- name: ListAllInventoryChangeLogs :many
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    icl.responsible_user_id,
    usr.name AS responsible_user_name
FROM inventory_change_log icl
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
WHERE icl.account_id = ?
AND (
    ? = false
    OR icl.item_id IN (/*SLICE:item_ids*/?)
)
AND (
    ? = false
    OR icl.action_type_code IN (/*SLICE:action_type_codes*/?)
)
AND (
    ? = false
    OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?)
)
AND (
    ? IS NULL
    OR icl.created_at >= ?
)
AND (
    ? IS NULL
    OR icl.created_at <= ?
)
ORDER BY icl.created_at DESC, icl.id DESC
`

type ListAllInventoryChangeLogsParams struct {
	AccountID               string
	IncludeItemFilter       interface{}
	ItemIds                 []string
	IncludeActionTypeFilter interface{}
	ActionTypeCodes         []string
	IncludeUserFilter       interface{}
	ChangedByUserIds        []sql.NullString
	StartDate               sql.NullTime
	EndDate                 sql.NullTime
}

type ListAllInventoryChangeLogsRow struct {
	ID                       string
	ActionTypeCode           string
	AccountID                string
	CreatedAt                time.Time
	UpdatedAt                time.Time
	ItemID                   string
	ItemSku                  string
	ItemTypeCode             string
	QuantityID               string
	QuantityValue            string
	QuantityUnitID           string
	QuantityUnitName         string
	QuantityUnitAbbreviation string
	QuantityUnitType         string
	ScanningStationID        sql.NullString
	ScanningStationName      sql.NullString
	ScanningStationType      sql.NullString
	ResponsibleUserID        sql.NullString
	ResponsibleUserName      sql.NullString
}

func (q *Queries) ListAllInventoryChangeLogs(ctx context.Context, arg ListAllInventoryChangeLogsParams) ([]ListAllInventoryChangeLogsRow, error) {
	query := listAllInventoryChangeLogs
	var queryParams []interface{}
	queryParams = append(queryParams, arg.AccountID)
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListAllInventoryChangeLogsRow
	for rows.Next() {
		var i ListAllInventoryChangeLogsRow
		if err := rows.Scan(
			&i.ID,
			&i.ActionTypeCode,
			&i.AccountID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.ItemID,
			&i.ItemSku,
			&i.ItemTypeCode,
			&i.QuantityID,
			&i.QuantityValue,
			&i.QuantityUnitID,
			&i.QuantityUnitName,
			&i.QuantityUnitAbbreviation,
			&i.QuantityUnitType,
			&i.ScanningStationID,
			&i.ScanningStationName,
			&i.ScanningStationType,
			&i.ResponsibleUserID,
			&i.ResponsibleUserName,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listConsumptionChangeLogsForBurnRate = `-- name: ListConsumptionChangeLogsForBurnRate :many
SELECT
    q.value,
    q.unit_id,
    icl.created_at
FROM inventory_change_log icl
JOIN quantity q ON q.id = icl.quantity_id
WHERE icl.item_id = ?
AND icl.account_id = ?
AND icl.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
AND icl.action_type_code IN ('scan', 'user_correction')
AND CAST(q.value AS DECIMAL(65,30)) < 0
ORDER BY icl.created_at ASC
`

type ListConsumptionChangeLogsForBurnRateParams struct {
	ItemID    string
	AccountID string
}

type ListConsumptionChangeLogsForBurnRateRow struct {
	Value     string
	UnitID    string
	CreatedAt time.Time
}

func (q *Queries) ListConsumptionChangeLogsForBurnRate(ctx context.Context, arg ListConsumptionChangeLogsForBurnRateParams) ([]ListConsumptionChangeLogsForBurnRateRow, error) {
	rows, err := q.db.QueryContext(ctx, listConsumptionChangeLogsForBurnRate, arg.ItemID, arg.AccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListConsumptionChangeLogsForBurnRateRow
	for rows.Next() {
		var i ListConsumptionChangeLogsForBurnRateRow
		if err := rows.Scan(&i.Value, &i.UnitID, &i.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listInventoryChangeLogsBackward = `-- name: ListInventoryChangeLogsBackward :many
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    i.created_at AS item_created_at,
    i.updated_at AS item_updated_at,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    u.ratio_numerator AS quantity_unit_ratio_numerator,
    u.ratio_denominator AS quantity_unit_ratio_denominator,
    u.offset_numerator AS quantity_unit_offset_numerator,
    u.offset_denominator AS quantity_unit_offset_denominator,
    u.created_at AS quantity_unit_created_at,
    u.updated_at AS quantity_unit_updated_at,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    ss.created_at AS scanning_station_created_at,
    ss.updated_at AS scanning_station_updated_at,
    icl.responsible_user_id,
    usr.name AS responsible_user_name,
    usr.created_at AS responsible_user_created_at,
    usr.updated_at AS responsible_user_updated_at
FROM inventory_change_log icl
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
WHERE icl.account_id = ?
AND (
    ? = false
    OR icl.item_id IN (/*SLICE:item_ids*/?)
)
AND (
    ? = false
    OR icl.action_type_code IN (/*SLICE:action_type_codes*/?)
)
AND (
    ? = false
    OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?)
)
AND (
    ? IS NULL
    OR icl.created_at >= ?
)
AND (
    ? IS NULL
    OR icl.created_at <= ?
)
AND (
    icl.created_at > ?
    OR (icl.created_at = ? AND icl.id > ?)
)
ORDER BY icl.created_at ASC, icl.id ASC
LIMIT ?
`

type ListInventoryChangeLogsBackwardParams struct {
	AccountID               string
	IncludeItemFilter       interface{}
	ItemIds                 []string
	IncludeActionTypeFilter interface{}
	ActionTypeCodes         []string
	IncludeUserFilter       interface{}
	ChangedByUserIds        []sql.NullString
	StartDate               sql.NullTime
	EndDate                 sql.NullTime
	CursorCreatedAt         time.Time
	CursorID                string
	Limit                   int32
}

type ListInventoryChangeLogsBackwardRow struct {
	ID                            string
	ActionTypeCode                string
	AccountID                     string
	CreatedAt                     time.Time
	UpdatedAt                     time.Time
	ItemID                        string
	ItemSku                       string
	ItemTypeCode                  string
	ItemCreatedAt                 time.Time
	ItemUpdatedAt                 time.Time
	QuantityID                    string
	QuantityValue                 string
	QuantityUnitID                string
	QuantityUnitName              string
	QuantityUnitAbbreviation      string
	QuantityUnitType              string
	QuantityUnitRatioNumerator    string
	QuantityUnitRatioDenominator  string
	QuantityUnitOffsetNumerator   string
	QuantityUnitOffsetDenominator string
	QuantityUnitCreatedAt         time.Time
	QuantityUnitUpdatedAt         time.Time
	ScanningStationID             sql.NullString
	ScanningStationName           sql.NullString
	ScanningStationType           sql.NullString
	ScanningStationCreatedAt      sql.NullTime
	ScanningStationUpdatedAt      sql.NullTime
	ResponsibleUserID             sql.NullString
	ResponsibleUserName           sql.NullString
	ResponsibleUserCreatedAt      sql.NullTime
	ResponsibleUserUpdatedAt      sql.NullTime
}

func (q *Queries) ListInventoryChangeLogsBackward(ctx context.Context, arg ListInventoryChangeLogsBackwardParams) ([]ListInventoryChangeLogsBackwardRow, error) {
	query := listInventoryChangeLogsBackward
	var queryParams []interface{}
	queryParams = append(queryParams, arg.AccountID)
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit)
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListInventoryChangeLogsBackwardRow
	for rows.Next() {
		var i ListInventoryChangeLogsBackwardRow
		if err := rows.Scan(
			&i.ID,
			&i.ActionTypeCode,
			&i.AccountID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.ItemID,
			&i.ItemSku,
			&i.ItemTypeCode,
			&i.ItemCreatedAt,
			&i.ItemUpdatedAt,
			&i.QuantityID,
			&i.QuantityValue,
			&i.QuantityUnitID,
			&i.QuantityUnitName,
			&i.QuantityUnitAbbreviation,
			&i.QuantityUnitType,
			&i.QuantityUnitRatioNumerator,
			&i.QuantityUnitRatioDenominator,
			&i.QuantityUnitOffsetNumerator,
			&i.QuantityUnitOffsetDenominator,
			&i.QuantityUnitCreatedAt,
			&i.QuantityUnitUpdatedAt,
			&i.ScanningStationID,
			&i.ScanningStationName,
			&i.ScanningStationType,
			&i.ScanningStationCreatedAt,
			&i.ScanningStationUpdatedAt,
			&i.ResponsibleUserID,
			&i.ResponsibleUserName,
			&i.ResponsibleUserCreatedAt,
			&i.ResponsibleUserUpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listInventoryChangeLogsForward = `-- name: ListInventoryChangeLogsForward :many
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    i.created_at AS item_created_at,
    i.updated_at AS item_updated_at,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    u.ratio_numerator AS quantity_unit_ratio_numerator,
    u.ratio_denominator AS quantity_unit_ratio_denominator,
    u.offset_numerator AS quantity_unit_offset_numerator,
    u.offset_denominator AS quantity_unit_offset_denominator,
    u.created_at AS quantity_unit_created_at,
    u.updated_at AS quantity_unit_updated_at,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    ss.created_at AS scanning_station_created_at,
    ss.updated_at AS scanning_station_updated_at,
    icl.responsible_user_id,
    usr.name AS responsible_user_name,
    usr.created_at AS responsible_user_created_at,
    usr.updated_at AS responsible_user_updated_at
FROM inventory_change_log icl
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
WHERE icl.account_id = ?
AND (
    ? = false
    OR icl.item_id IN (/*SLICE:item_ids*/?)
)
AND (
    ? = false
    OR icl.action_type_code IN (/*SLICE:action_type_codes*/?)
)
AND (
    ? = false
    OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?)
)
AND (
    ? IS NULL
    OR icl.created_at >= ?
)
AND (
    ? IS NULL
    OR icl.created_at <= ?
)
AND (
    ? IS NULL
    OR icl.created_at < ?
    OR (icl.created_at = ? AND icl.id < ?)
)
ORDER BY icl.created_at DESC, icl.id DESC
LIMIT ?
`

type ListInventoryChangeLogsForwardParams struct {
	AccountID               string
	IncludeItemFilter       interface{}
	ItemIds                 []string
	IncludeActionTypeFilter interface{}
	ActionTypeCodes         []string
	IncludeUserFilter       interface{}
	ChangedByUserIds        []sql.NullString
	StartDate               sql.NullTime
	EndDate                 sql.NullTime
	CursorCreatedAt         sql.NullTime
	CursorID                sql.NullString
	Limit                   int32
}

type ListInventoryChangeLogsForwardRow struct {
	ID                            string
	ActionTypeCode                string
	AccountID                     string
	CreatedAt                     time.Time
	UpdatedAt                     time.Time
	ItemID                        string
	ItemSku                       string
	ItemTypeCode                  string
	ItemCreatedAt                 time.Time
	ItemUpdatedAt                 time.Time
	QuantityID                    string
	QuantityValue                 string
	QuantityUnitID                string
	QuantityUnitName              string
	QuantityUnitAbbreviation      string
	QuantityUnitType              string
	QuantityUnitRatioNumerator    string
	QuantityUnitRatioDenominator  string
	QuantityUnitOffsetNumerator   string
	QuantityUnitOffsetDenominator string
	QuantityUnitCreatedAt         time.Time
	QuantityUnitUpdatedAt         time.Time
	ScanningStationID             sql.NullString
	ScanningStationName           sql.NullString
	ScanningStationType           sql.NullString
	ScanningStationCreatedAt      sql.NullTime
	ScanningStationUpdatedAt      sql.NullTime
	ResponsibleUserID             sql.NullString
	ResponsibleUserName           sql.NullString
	ResponsibleUserCreatedAt      sql.NullTime
	ResponsibleUserUpdatedAt      sql.NullTime
}

func (q *Queries) ListInventoryChangeLogsForward(ctx context.Context, arg ListInventoryChangeLogsForwardParams) ([]ListInventoryChangeLogsForwardRow, error) {
	query := listInventoryChangeLogsForward
	var queryParams []interface{}
	queryParams = append(queryParams, arg.AccountID)
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit)
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListInventoryChangeLogsForwardRow
	for rows.Next() {
		var i ListInventoryChangeLogsForwardRow
		if err := rows.Scan(
			&i.ID,
			&i.ActionTypeCode,
			&i.AccountID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.ItemID,
			&i.ItemSku,
			&i.ItemTypeCode,
			&i.ItemCreatedAt,
			&i.ItemUpdatedAt,
			&i.QuantityID,
			&i.QuantityValue,
			&i.QuantityUnitID,
			&i.QuantityUnitName,
			&i.QuantityUnitAbbreviation,
			&i.QuantityUnitType,
			&i.QuantityUnitRatioNumerator,
			&i.QuantityUnitRatioDenominator,
			&i.QuantityUnitOffsetNumerator,
			&i.QuantityUnitOffsetDenominator,
			&i.QuantityUnitCreatedAt,
			&i.QuantityUnitUpdatedAt,
			&i.ScanningStationID,
			&i.ScanningStationName,
			&i.ScanningStationType,
			&i.ScanningStationCreatedAt,
			&i.ScanningStationUpdatedAt,
			&i.ResponsibleUserID,
			&i.ResponsibleUserName,
			&i.ResponsibleUserCreatedAt,
			&i.ResponsibleUserUpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const resolveInventoryChangeLogSearchItemIDs = `-- name: ResolveInventoryChangeLogSearchItemIDs :many
SELECT id FROM item WHERE account_id = ? AND sku LIKE ?
`

type ResolveInventoryChangeLogSearchItemIDsParams struct {
	AccountID   string
	SearchQuery string
}

// ResolveInventoryChangeLogSearchItemIDs finds the items whose SKU matches a search term, so the list query can filter on item_id instead of joining item and matching a LIKE.
func (q *Queries) ResolveInventoryChangeLogSearchItemIDs(ctx context.Context, arg ResolveInventoryChangeLogSearchItemIDsParams) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, resolveInventoryChangeLogSearchItemIDs, arg.AccountID, arg.SearchQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []string
	for rows.Next() {
		var id string
		if err := rows.Scan(&id); err != nil {
			return nil, err
		}
		items = append(items, id)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const resolveInventoryChangeLogSearchStationIDs = `-- name: ResolveInventoryChangeLogSearchStationIDs :many
SELECT id FROM scanning_station WHERE account_id = ? AND name LIKE ?
`

type ResolveInventoryChangeLogSearchStationIDsParams struct {
	AccountID   string
	SearchQuery string
}

// ResolveInventoryChangeLogSearchStationIDs finds the scanning stations whose name matches a search term.
func (q *Queries) ResolveInventoryChangeLogSearchStationIDs(ctx context.Context, arg ResolveInventoryChangeLogSearchStationIDsParams) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, resolveInventoryChangeLogSearchStationIDs, arg.AccountID, arg.SearchQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []string
	for rows.Next() {
		var id string
		if err := rows.Scan(&id); err != nil {
			return nil, err
		}
		items = append(items, id)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const resolveInventoryChangeLogSearchUserIDs = `-- name: ResolveInventoryChangeLogSearchUserIDs :many
SELECT id FROM user WHERE name LIKE ?
`

// ResolveInventoryChangeLogSearchUserIDs finds the users whose name matches a search term.
//
// Not account-scoped: the log row's own account_id does the scoping, and a name matched against a user outside the account only yields an id that no in-account row references.
func (q *Queries) ResolveInventoryChangeLogSearchUserIDs(ctx context.Context, searchQuery sql.NullString) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, resolveInventoryChangeLogSearchUserIDs, searchQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []string
	for rows.Next() {
		var id string
		if err := rows.Scan(&id); err != nil {
			return nil, err
		}
		items = append(items, id)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchInventoryChangeLogsBackward = `-- name: SearchInventoryChangeLogsBackward :many
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    i.created_at AS item_created_at,
    i.updated_at AS item_updated_at,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    u.ratio_numerator AS quantity_unit_ratio_numerator,
    u.ratio_denominator AS quantity_unit_ratio_denominator,
    u.offset_numerator AS quantity_unit_offset_numerator,
    u.offset_denominator AS quantity_unit_offset_denominator,
    u.created_at AS quantity_unit_created_at,
    u.updated_at AS quantity_unit_updated_at,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    ss.created_at AS scanning_station_created_at,
    ss.updated_at AS scanning_station_updated_at,
    icl.responsible_user_id,
    usr.name AS responsible_user_name,
    usr.created_at AS responsible_user_created_at,
    usr.updated_at AS responsible_user_updated_at
FROM (
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.item_id IN (/*SLICE:search_item_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          icl.created_at > ?
          OR (icl.created_at = ? AND icl.id > ?)
      )
     ORDER BY icl.created_at ASC, icl.id ASC
     LIMIT ?)
    UNION DISTINCT
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.responsible_user_id IN (/*SLICE:search_user_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          icl.created_at > ?
          OR (icl.created_at = ? AND icl.id > ?)
      )
     ORDER BY icl.created_at ASC, icl.id ASC
     LIMIT ?)
    UNION DISTINCT
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.scanning_station_id IN (/*SLICE:search_station_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          icl.created_at > ?
          OR (icl.created_at = ? AND icl.id > ?)
      )
     ORDER BY icl.created_at ASC, icl.id ASC
     LIMIT ?)
) matched
JOIN inventory_change_log icl ON icl.id = matched.id
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
ORDER BY icl.created_at ASC, icl.id ASC
LIMIT ?
`

type SearchInventoryChangeLogsBackwardParams struct {
	AccountID               string
	SearchItemIds           []string
	IncludeItemFilter       interface{}
	ItemIds                 []string
	IncludeActionTypeFilter interface{}
	ActionTypeCodes         []string
	IncludeUserFilter       interface{}
	ChangedByUserIds        []sql.NullString
	StartDate               sql.NullTime
	EndDate                 sql.NullTime
	CursorCreatedAt         time.Time
	CursorID                string
	Limit                   int32
	SearchUserIds           []sql.NullString
	Limit_2                 int32
	SearchStationIds        []sql.NullString
	Limit_3                 int32
	Limit_4                 int32
}

type SearchInventoryChangeLogsBackwardRow struct {
	ID                            string
	ActionTypeCode                string
	AccountID                     string
	CreatedAt                     time.Time
	UpdatedAt                     time.Time
	ItemID                        string
	ItemSku                       string
	ItemTypeCode                  string
	ItemCreatedAt                 time.Time
	ItemUpdatedAt                 time.Time
	QuantityID                    string
	QuantityValue                 string
	QuantityUnitID                string
	QuantityUnitName              string
	QuantityUnitAbbreviation      string
	QuantityUnitType              string
	QuantityUnitRatioNumerator    string
	QuantityUnitRatioDenominator  string
	QuantityUnitOffsetNumerator   string
	QuantityUnitOffsetDenominator string
	QuantityUnitCreatedAt         time.Time
	QuantityUnitUpdatedAt         time.Time
	ScanningStationID             sql.NullString
	ScanningStationName           sql.NullString
	ScanningStationType           sql.NullString
	ScanningStationCreatedAt      sql.NullTime
	ScanningStationUpdatedAt      sql.NullTime
	ResponsibleUserID             sql.NullString
	ResponsibleUserName           sql.NullString
	ResponsibleUserCreatedAt      sql.NullTime
	ResponsibleUserUpdatedAt      sql.NullTime
}

// SearchInventoryChangeLogsBackward is SearchInventoryChangeLogsForward walking the other way, for a backward cursor. See that query for why the search is a union of resolved-id branches.
func (q *Queries) SearchInventoryChangeLogsBackward(ctx context.Context, arg SearchInventoryChangeLogsBackwardParams) ([]SearchInventoryChangeLogsBackwardRow, error) {
	query := searchInventoryChangeLogsBackward
	var queryParams []interface{}
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchItemIds) > 0 {
		for _, v := range arg.SearchItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_item_ids*/?", strings.Repeat(",?", len(arg.SearchItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit)
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchUserIds) > 0 {
		for _, v := range arg.SearchUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_user_ids*/?", strings.Repeat(",?", len(arg.SearchUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit_2)
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchStationIds) > 0 {
		for _, v := range arg.SearchStationIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_station_ids*/?", strings.Repeat(",?", len(arg.SearchStationIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_station_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit_3)
	queryParams = append(queryParams, arg.Limit_4)
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchInventoryChangeLogsBackwardRow
	for rows.Next() {
		var i SearchInventoryChangeLogsBackwardRow
		if err := rows.Scan(
			&i.ID,
			&i.ActionTypeCode,
			&i.AccountID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.ItemID,
			&i.ItemSku,
			&i.ItemTypeCode,
			&i.ItemCreatedAt,
			&i.ItemUpdatedAt,
			&i.QuantityID,
			&i.QuantityValue,
			&i.QuantityUnitID,
			&i.QuantityUnitName,
			&i.QuantityUnitAbbreviation,
			&i.QuantityUnitType,
			&i.QuantityUnitRatioNumerator,
			&i.QuantityUnitRatioDenominator,
			&i.QuantityUnitOffsetNumerator,
			&i.QuantityUnitOffsetDenominator,
			&i.QuantityUnitCreatedAt,
			&i.QuantityUnitUpdatedAt,
			&i.ScanningStationID,
			&i.ScanningStationName,
			&i.ScanningStationType,
			&i.ScanningStationCreatedAt,
			&i.ScanningStationUpdatedAt,
			&i.ResponsibleUserID,
			&i.ResponsibleUserName,
			&i.ResponsibleUserCreatedAt,
			&i.ResponsibleUserUpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchInventoryChangeLogsForward = `-- name: SearchInventoryChangeLogsForward :many
SELECT
    icl.id,
    icl.action_type_code,
    icl.account_id,
    icl.created_at,
    icl.updated_at,
    i.id AS item_id,
    i.sku AS item_sku,
    i.item_type_code AS item_type_code,
    i.created_at AS item_created_at,
    i.updated_at AS item_updated_at,
    q.id AS quantity_id,
    q.value AS quantity_value,
    u.id AS quantity_unit_id,
    u.name AS quantity_unit_name,
    u.abbreviation AS quantity_unit_abbreviation,
    u.unit_dimension_code AS quantity_unit_type,
    u.ratio_numerator AS quantity_unit_ratio_numerator,
    u.ratio_denominator AS quantity_unit_ratio_denominator,
    u.offset_numerator AS quantity_unit_offset_numerator,
    u.offset_denominator AS quantity_unit_offset_denominator,
    u.created_at AS quantity_unit_created_at,
    u.updated_at AS quantity_unit_updated_at,
    icl.scanning_station_id,
    ss.name AS scanning_station_name,
    ss.scanning_station_type_code AS scanning_station_type,
    ss.created_at AS scanning_station_created_at,
    ss.updated_at AS scanning_station_updated_at,
    icl.responsible_user_id,
    usr.name AS responsible_user_name,
    usr.created_at AS responsible_user_created_at,
    usr.updated_at AS responsible_user_updated_at
FROM (
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.item_id IN (/*SLICE:search_item_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          ? IS NULL
          OR icl.created_at < ?
          OR (icl.created_at = ? AND icl.id < ?)
      )
     ORDER BY icl.created_at DESC, icl.id DESC
     LIMIT ?)
    UNION DISTINCT
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.responsible_user_id IN (/*SLICE:search_user_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          ? IS NULL
          OR icl.created_at < ?
          OR (icl.created_at = ? AND icl.id < ?)
      )
     ORDER BY icl.created_at DESC, icl.id DESC
     LIMIT ?)
    UNION DISTINCT
    (SELECT icl.id AS id, icl.created_at AS created_at
     FROM inventory_change_log icl
     WHERE icl.account_id = ?
      AND icl.scanning_station_id IN (/*SLICE:search_station_ids*/?)
      AND (? = false OR icl.item_id IN (/*SLICE:item_ids*/?))
      AND (? = false OR icl.action_type_code IN (/*SLICE:action_type_codes*/?))
      AND (? = false OR icl.responsible_user_id IN (/*SLICE:changed_by_user_ids*/?))
      AND (? IS NULL OR icl.created_at >= ?)
      AND (? IS NULL OR icl.created_at <= ?)
      AND (
          ? IS NULL
          OR icl.created_at < ?
          OR (icl.created_at = ? AND icl.id < ?)
      )
     ORDER BY icl.created_at DESC, icl.id DESC
     LIMIT ?)
) matched
JOIN inventory_change_log icl ON icl.id = matched.id
JOIN item i ON i.id = icl.item_id
JOIN quantity q ON q.id = icl.quantity_id
JOIN unit u ON u.id = q.unit_id
LEFT JOIN scanning_station ss ON ss.id = icl.scanning_station_id
LEFT JOIN user usr ON usr.id = icl.responsible_user_id
ORDER BY icl.created_at DESC, icl.id DESC
LIMIT ?
`

type SearchInventoryChangeLogsForwardParams struct {
	AccountID               string
	SearchItemIds           []string
	IncludeItemFilter       interface{}
	ItemIds                 []string
	IncludeActionTypeFilter interface{}
	ActionTypeCodes         []string
	IncludeUserFilter       interface{}
	ChangedByUserIds        []sql.NullString
	StartDate               sql.NullTime
	EndDate                 sql.NullTime
	CursorCreatedAt         sql.NullTime
	CursorID                sql.NullString
	Limit                   int32
	SearchUserIds           []sql.NullString
	Limit_2                 int32
	SearchStationIds        []sql.NullString
	Limit_3                 int32
	Limit_4                 int32
}

type SearchInventoryChangeLogsForwardRow struct {
	ID                            string
	ActionTypeCode                string
	AccountID                     string
	CreatedAt                     time.Time
	UpdatedAt                     time.Time
	ItemID                        string
	ItemSku                       string
	ItemTypeCode                  string
	ItemCreatedAt                 time.Time
	ItemUpdatedAt                 time.Time
	QuantityID                    string
	QuantityValue                 string
	QuantityUnitID                string
	QuantityUnitName              string
	QuantityUnitAbbreviation      string
	QuantityUnitType              string
	QuantityUnitRatioNumerator    string
	QuantityUnitRatioDenominator  string
	QuantityUnitOffsetNumerator   string
	QuantityUnitOffsetDenominator string
	QuantityUnitCreatedAt         time.Time
	QuantityUnitUpdatedAt         time.Time
	ScanningStationID             sql.NullString
	ScanningStationName           sql.NullString
	ScanningStationType           sql.NullString
	ScanningStationCreatedAt      sql.NullTime
	ScanningStationUpdatedAt      sql.NullTime
	ResponsibleUserID             sql.NullString
	ResponsibleUserName           sql.NullString
	ResponsibleUserCreatedAt      sql.NullTime
	ResponsibleUserUpdatedAt      sql.NullTime
}

// SearchInventoryChangeLogsForward is the free-text page: the same list as ListInventoryChangeLogsForward, restricted to rows matching a search term.
//
// The term is resolved to id sets on item, user and scanning_station before this runs, so the predicate here is an equality on an indexed column rather than a LIKE. That matters more than it looks: matching with `i.sku LIKE ? OR usr.name LIKE ? OR ss.name LIKE ?` across the joins reaches no index at all, and a term matching nothing walks the account's entire log - 47s on the largest tenant.
// One branch per dimension, unioned, rather than one OR over the three columns: an OR reaches no single index either and cost 24s on that same tenant, where the union costs 0.13s. Each branch drives its own (account_id, <dimension>, created_at DESC, id DESC) composite.
// Each branch is cut to the page limit and the merge is cut again. Taking the top page from each branch is enough to build the true top page, because every branch carries the same sort - a row in the global page is necessarily in its own branch's page.
// A dimension the term did not match arrives as an empty set, which the generated SQL renders as `IN (NULL)`; the branch is then an impossible seek rather than a scan. When no dimension matches at all the caller skips this query entirely and returns an empty page.
func (q *Queries) SearchInventoryChangeLogsForward(ctx context.Context, arg SearchInventoryChangeLogsForwardParams) ([]SearchInventoryChangeLogsForwardRow, error) {
	query := searchInventoryChangeLogsForward
	var queryParams []interface{}
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchItemIds) > 0 {
		for _, v := range arg.SearchItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_item_ids*/?", strings.Repeat(",?", len(arg.SearchItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit)
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchUserIds) > 0 {
		for _, v := range arg.SearchUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_user_ids*/?", strings.Repeat(",?", len(arg.SearchUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit_2)
	queryParams = append(queryParams, arg.AccountID)
	if len(arg.SearchStationIds) > 0 {
		for _, v := range arg.SearchStationIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:search_station_ids*/?", strings.Repeat(",?", len(arg.SearchStationIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:search_station_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeItemFilter)
	if len(arg.ItemIds) > 0 {
		for _, v := range arg.ItemIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:item_ids*/?", strings.Repeat(",?", len(arg.ItemIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:item_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeActionTypeFilter)
	if len(arg.ActionTypeCodes) > 0 {
		for _, v := range arg.ActionTypeCodes {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", strings.Repeat(",?", len(arg.ActionTypeCodes))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:action_type_codes*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.IncludeUserFilter)
	if len(arg.ChangedByUserIds) > 0 {
		for _, v := range arg.ChangedByUserIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", strings.Repeat(",?", len(arg.ChangedByUserIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:changed_by_user_ids*/?", "NULL", 1)
	}
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.StartDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.EndDate)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorCreatedAt)
	queryParams = append(queryParams, arg.CursorID)
	queryParams = append(queryParams, arg.Limit_3)
	queryParams = append(queryParams, arg.Limit_4)
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchInventoryChangeLogsForwardRow
	for rows.Next() {
		var i SearchInventoryChangeLogsForwardRow
		if err := rows.Scan(
			&i.ID,
			&i.ActionTypeCode,
			&i.AccountID,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.ItemID,
			&i.ItemSku,
			&i.ItemTypeCode,
			&i.ItemCreatedAt,
			&i.ItemUpdatedAt,
			&i.QuantityID,
			&i.QuantityValue,
			&i.QuantityUnitID,
			&i.QuantityUnitName,
			&i.QuantityUnitAbbreviation,
			&i.QuantityUnitType,
			&i.QuantityUnitRatioNumerator,
			&i.QuantityUnitRatioDenominator,
			&i.QuantityUnitOffsetNumerator,
			&i.QuantityUnitOffsetDenominator,
			&i.QuantityUnitCreatedAt,
			&i.QuantityUnitUpdatedAt,
			&i.ScanningStationID,
			&i.ScanningStationName,
			&i.ScanningStationType,
			&i.ScanningStationCreatedAt,
			&i.ScanningStationUpdatedAt,
			&i.ResponsibleUserID,
			&i.ResponsibleUserName,
			&i.ResponsibleUserCreatedAt,
			&i.ResponsibleUserUpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
