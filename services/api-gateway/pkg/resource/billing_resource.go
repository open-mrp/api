...
const SampleStripeCustomerID = "cus_abc123xyz789"
const SampleSubscriptionID = "sub_abc123xyz789"
const SampleBillingPortalURL = "https://billing.stripe.com/p/session/test_abc123xyz789"
const SampleCheckoutURL = "https://checkout.stripe.com/c/pay/cs_test_abc123xyz789"
const SampleClientSecret = "cs_test_secret_abc123xyz789"
const SamplePublishableKey = "pk_test_abc123xyz789"

var SampleUsageItem = &UsageItem{
	Current: 5,
	Limit:   ptrutil.Ptr(10),
}
...
var SampleSubscriptionInfo = &SubscriptionInfo{
	Status:            "trialing",
	CurrentPeriodEnd:  ptrutil.Ptr(time.Now().AddDate(0, 0, 14)),
	TrialEnd:          ptrutil.Ptr(time.Now().AddDate(0, 0, 14)),
...
var SampleSwitchPlanResponse = &SwitchPlanResponse{
	...
	CheckoutURL:     ptrutil.Ptr(SampleCheckoutURL),
}
...
type ProrationPreview struct {
	// The credit amount in cents from unused time on current plan
	CreditAmount int64 `json:"credit_amount"`
	// The charge amount in cents for the new plan
	ChargeAmount int64 `json:"charge_amount"`
	// The net amount in cents (charge - credit)
