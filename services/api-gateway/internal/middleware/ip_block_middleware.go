...
// blockedIPs is the hardcoded set of IP addresses that are denied access to
// every router. Entries should be exact-match IP literals (IPv4 or IPv6).
...
// IPBlockMiddleware rejects requests whose client IP is on the block list.
// It runs early in the chain so blocked traffic is not rate-limited, authenticated,
