// Command replay-inventory-received re-drives core.inventory_received_allocation inbox messages that
// failed permanently, one at a time, in the order they were originally received.
//
// A run of these messages had its transaction aborted by Vitess when the (then inline) open-issue
// allocation walk ran past the transaction time limit, so nothing was applied: the inbox rows sit at
// status 'received' with processed_at NULL and attempts exhausted, while the payloads survive intact
// in message_outbox. The handler now only enqueues the paged allocate-open-issues consumer, so a
// replayed message no longer does the walk inline; this catches up the messages the old handler
// stranded.
//
...
// Usage:
//
//	DB_URL=<dsn-or-mysql-url> go run ./services/core-service/cmd/replay-inventory-received --account <id> --dry-run
//	DB_URL=<dsn-or-mysql-url> go run ./services/core-service/cmd/replay-inventory-received --account <id>
//
// Flags:
//
...
const replayHandler = "core.inventory_received_allocation"

const replayRoutingKey = "core.event.inventory_received"
...
func Run(
	...
	fs := flag.NewFlagSet("replay-inventory-received", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	if len(messages) == 0 {
		fmt.Fprintf(stdout, "no permanently-failed inventory_received messages for account %s; nothing to do\n", *account)
		return nil
	}

	if *dryRun {
		fmt.Fprintf(stdout, "would replay %d message(s) for account %s, in order:\n", len(messages), *account)
		fmt.Fprintf(stdout, "  %-4s  %-24s  %-38s  %-6s  %s\n", "#", "received_at", "message_id", "items", "reason")
		for i, m := range messages {
			fmt.Fprintf(stdout, "  %-4d  %-24s  %-38s  %-6d  %s\n",
				i+1, m.receivedAt.UTC().Format(time.RFC3339), m.messageID, len(m.itemIDs), m.reason)
		}
		return nil
	}
	...
	consumer := event.NewInventoryReceivedConsumer(nil, inboxRepo, repoFactory, txManager)
...
// loadFailedMessages returns the permanently-failed inventory_received messages for one account,
// oldest first, capped at limit. The inbox is joined to the outbox for the surviving payload, and the
// account and display fields are read from the decoded event because the inbox row does not carry them.
...
// decodeEvent unwraps the AMQP envelope stored in the outbox payload and returns the inventory-received
// event it carries. The envelope's data field is base64-encoded JSON, which json.Unmarshal into the
// []byte field decodes for us.
func decodeEvent(payload []byte) (domain.InventoryReceivedEvent, error) {
	var envelope contracts.AmqpMessage
	if err := json.Unmarshal(payload, &envelope); err != nil {
		return domain.InventoryReceivedEvent{}, fmt.Errorf("unmarshaling envelope: %w", err)
	}
	var evt domain.InventoryReceivedEvent
	if err := json.Unmarshal(envelope.Data, &evt); err != nil {
		return domain.InventoryReceivedEvent{}, fmt.Errorf("unmarshaling event: %w", err)
	}
