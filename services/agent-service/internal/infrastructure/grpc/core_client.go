...
func (c *AgentCoreClient) SearchProducts(ctx context.Context, accountID, query string) ([]domain.ProductResult, error) {
	ctx = prepareCtx(ctx)

	resp, apiErr := rpc.CallRPC(ctx, coreClientTracer, "core_client.search_products", coreServiceName,
		func(ctx context.Context, opts ...grpclib.CallOption) (*pb.SearchProductsResponse, error) {
			return c.client.SearchProducts(ctx, &pb.SearchProductsRequest{
				AccountId: accountID,
				Query:     query,
			}, opts...)
		})
	...
	products := make([]domain.ProductResult, len(resp.Products))
	for i, p := range resp.Products {
		products[i] = domain.ProductResult{
...
func (c *AgentCoreClient) ListProducts(ctx context.Context, accountID string) ([]domain.ProductResult, error) {
	ctx = prepareCtx(ctx)

	resp, apiErr := rpc.CallRPC(ctx, coreClientTracer, "core_client.list_products", coreServiceName,
		func(ctx context.Context, opts ...grpclib.CallOption) (*pb.ListProductsResponse, error) {
...
func (c *AgentCoreClient) GetCustomerByEmail(ctx context.Context, ownerAccountID, email string) (*domain.CustomerResult, error) {
	ctx = prepareCtx(ctx)

	resp, apiErr := rpc.CallRPC(ctx, coreClientTracer, "core_client.get_customer_by_email", coreServiceName,
		func(ctx context.Context, opts ...grpclib.CallOption) (*pb.GetCustomerByEmailResponse, error) {
			return c.client.GetCustomerByEmail(ctx, &pb.GetCustomerByEmailRequest{
	...
	return &domain.CustomerResult{
		RelationID:            resp.Customer.RelationId,
		OwnerAccountID:        resp.Customer.OwnerAccountId,
		CounterpartyAccountID: resp.Customer.CounterpartyAccountId,
		RoleCode:              resp.Customer.RoleCode,
		Alias:                 resp.Customer.Alias,
		Email:                 resp.Customer.Email,
		UserName:              resp.Customer.UserName,
	}, nil
}
