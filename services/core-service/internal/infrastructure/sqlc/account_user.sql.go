...
const getOwnerRoleIDByAccountID = `-- name: GetOwnerRoleIDByAccountID :one
SELECT r.id
FROM role r
...
func (q *Queries) GetOwnerRoleIDByAccountID(ctx context.Context, accountID sql.NullString) (string, error) {
	row := q.queryRow(ctx, q.getOwnerRoleIDByAccountIDStmt, getOwnerRoleIDByAccountID, accountID)
	var id string
	err := row.Scan(&id)
	return id, err
}
