...
// IncludeForcer is an optional interface a Transformer can implement to
// declare include keys that must be resolved on the latest-version response
// before the downgrade runs, so the transformer has the data it needs (e.g.
// expanding `user` so its fields can be hoisted back onto the parent).
type IncludeForcer interface {
	// ForcedIncludes returns include keys (dot-paths relative to the root
	// object type) that must be resolved when downgrading a response of the
	// given object type.
...
// ForcedIncludes collects the include keys required by every transformer that
// would run when downgrading a response of the given object type from the
