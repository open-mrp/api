...
const (
	...
	// defaultInitialWait is the base delay before the first retry (attempt 0). Subsequent attempts multiply this by Multiplier^attempt.
	defaultInitialWait = 1 * time.Second

	// defaultMaxWait caps the computed delay so that high attempt counts don't produce unreasonably long waits.
	defaultMaxWait = 10 * time.Second

	// defaultMultiplier is the exponential growth factor applied per attempt. A value of 2.0 produces the classic doubling backoff: 1s, 2s, 4s, 8s, …
	defaultMultiplier = 2.0

	// defaultJitterFraction adds ±10% randomness to each delay to prevent thundering-herd effects when many processes retry the same operation (e.g. a fleet of enqueuer instances all failing against the same DB).
...
// validate checks that the Config fields form a coherent retry policy. It returns a descriptive error for any constraint violation:
//   - MaxRetries must be >= 0 (WithDefaults replaces 0 with the default of 3 before WithBackoff validates, so the effective value is always >= 1).
...
// cryptoRandFloat64 returns a cryptographically random float64 in [0, 1). It uses crypto/rand instead of math/rand to avoid global-lock contention and to eliminate the need for seeding. If the entropy source fails (extremely unlikely), it falls back to 0.5 to provide a neutral jitter value rather than panicking.
