...
// RequireSandboxAccount returns an invariant violation error if the account is
// not a sandbox. Used before sandbox-mutating operations (delete, purge).
...
// RequireNotSandboxAccount returns a validation error if the account IS a
// sandbox. Used to block sandbox-from-sandbox creation.
