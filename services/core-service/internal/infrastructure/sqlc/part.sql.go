...
const partUpdateItem = `-- name: PartUpdateItem :execresult
UPDATE item SET
    sku = COALESCE(?, sku),
    description = ?,
    notes = ?,
...
func (q *Queries) PartUpdateItem(ctx context.Context, arg PartUpdateItemParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, partUpdateItem,
		arg.Sku,
		arg.Description,
		arg.Notes,
		arg.ID,
