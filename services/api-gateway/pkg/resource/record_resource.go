...
// Record is a lightweight reference to a business record — a sales order,
// purchase order, pick, shipment, production run, invoice, etc. Like Actor and
// Entity, it carries just enough to identify and label the referenced record
// without embedding its full resource. The optional status and metadata fields
// hold type-specific detail that varies by the kind of record referenced.
type Record struct {
	// Record ID.
