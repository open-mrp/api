...
const getUnitGroupsForCategoriesByIDs = `-- name: GetUnitGroupsForCategoriesByIDs :many
SELECT
    ug.id,
    ug.name,
    ug.base_unit_id,
...
type GetUnitGroupsForCategoriesByIDsRow struct {
	ID           string
	Name         string
	BaseUnitID   string
	UnitTypeCode string
...
func (q *Queries) GetUnitGroupsForCategoriesByIDs(ctx context.Context, ids []string) ([]GetUnitGroupsForCategoriesByIDsRow, error) {
	query := getUnitGroupsForCategoriesByIDs
	var queryParams []interface{}
	if len(ids) > 0 {
		for _, v := range ids {
			queryParams = append(queryParams, v)
	...
	defer rows.Close()
	var items []GetUnitGroupsForCategoriesByIDsRow
	for rows.Next() {
		var i GetUnitGroupsForCategoriesByIDsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.BaseUnitID,
