...
const (
	// OutboxStatusPending means the message has been written to the outbox table (inside the service's business transaction) and is waiting for the Enqueuer to pick it up.
	OutboxStatusPending OutboxStatus = "pending"
	// OutboxStatusPublished means the Enqueuer successfully published the message to RabbitMQ and the outbox record has been deleted (or marked) as complete.
	OutboxStatusPublished OutboxStatus = "published"
	// OutboxStatusFailed means the Enqueuer attempted to publish but encountered an error. The attempt count is incremented and the message is scheduled for retry with exponential backoff until MaxAttempts is reached.
