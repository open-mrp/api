...
func TestShippingTerms_List(t *testing.T) {
	...
	for _, item := range list.Data {
		if DataItemField(item, "id") == SeedShippingTermID {
			found = true
			break
		}
	}
	assert.True(t, found, "Seeded shipping term should appear in list")
}
...
func TestShippingTerms_ListCursorPagination(t *testing.T) {
	t.Parallel()
	page1, _, err := apiClient.GetList(shippingTermsPath, url.Values{"limit": {"1"}})
	require.NoError(t, err)
	requirePageLen(t, page1.Data, 1)

	if !page1.PageInfo.HasNextPage {
		t.Fatal("Not enough shipping terms for pagination test")
		return
	}
