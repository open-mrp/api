...
const findOpenIssuesForItem = `-- name: FindOpenIssuesForItem :many
SELECT
    ii.id,
    q.id AS quantity_id,
...
type FindOpenIssuesForItemRow struct {
	ID                string
	QuantityID        string
...
func (q *Queries) FindOpenIssuesForItem(ctx context.Context, arg FindOpenIssuesForItemParams) ([]FindOpenIssuesForItemRow, error) {
	...
	defer rows.Close()
	var items []FindOpenIssuesForItemRow
	for rows.Next() {
		var i FindOpenIssuesForItemRow
		if err := rows.Scan(
			&i.ID,
			&i.QuantityID,
			&i.QuantityValue,
