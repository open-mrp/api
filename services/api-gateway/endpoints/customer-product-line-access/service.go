...
func NewCustomerProductLineAccessSvc(config *CustomerProductLineAccessSvcConfig) CustomerProductLineAccessSvc {
	...
	return &customerProductLineAccessSvcImpl{coreClient: config.CoreClient}
}
...
func (m *customerProductLineAccessSvcImpl) ListCustomerProductLineAccess(ctx context.Context, req *ListCustomerProductLineAccessRequest) (*apiresource.List[apiresource.CustomerProductLineAccess], *apierror.APIError) {
	...
	loaded, apiErr := resourceloaders.LoadCustomerProductLineAccess(ctx, ids)
	if apiErr != nil {
		return nil, apiErr
	}
	items := make([]apiresource.CustomerProductLineAccess, 0, len(ids))
	for _, id := range ids {
		if v, ok := loaded[id]; ok {
			items = append(items, *(v.(*apiresource.CustomerProductLineAccess)))
		}
	}
...
func (m *customerProductLineAccessSvcImpl) GetCustomerProductLineAccess(ctx context.Context, req *RetrieveCustomerProductLineAccessRequest) (*apiresource.CustomerProductLineAccess, *apierror.APIError) {
	return loadCustomerProductLineAccessByID(ctx, req.CustomerID)
}
...
func (m *customerProductLineAccessSvcImpl) CreateCustomerProductLineAccess(ctx context.Context, req *CreateCustomerProductLineAccessRequest) (*apiresource.CustomerProductLineAccess, *apierror.APIError) {
	...
	return loadCustomerProductLineAccessByID(ctx, resp.Item.CustomerId)
}
...
func (m *customerProductLineAccessSvcImpl) DeleteCustomerProductLineAccess(ctx context.Context, req *DeleteCustomerProductLineAccessRequest) (*apiresource.EmptyResource, *apierror.APIError) {
	pbReq := &pb.DeleteCustomerProductLineAccessRequest{CustomerId: req.CustomerID}
...
// loadCustomerProductLineAccessByID wraps the single-ID load pattern used after
// mutations and for the retrieve endpoint. Keyed by customer_id.
func loadCustomerProductLineAccessByID(ctx context.Context, customerID string) (*apiresource.CustomerProductLineAccess, *apierror.APIError) {
	loaded, apiErr := resourceloaders.LoadCustomerProductLineAccess(ctx, []string{customerID})
	if apiErr != nil {
		return nil, apiErr
	}
	v, ok := loaded[customerID]
	if !ok {
		return nil, apierror.NewResourceNotFoundError("Customer product line access not found.")
	}
	return v.(*apiresource.CustomerProductLineAccess), nil
}
