...
// newServerHandler creates an otelgrpc server stats handler that creates a span for each inbound RPC. Health-check RPCs (grpc.health.v1.Health/Check) are filtered out to avoid noisy traces from Kubernetes probes.
...
// grpcSpanNameFormatter converts a gRPC full method name (e.g. "/auth.v1.AuthService/LoginUser") into a concise span name following the "grpc.<snake_case_method>" convention (e.g. "grpc.login_user"). It strips the leading slash and package/service prefix, then converts the remaining PascalCase method name to snake_case via [toSnakeLower].
...
// isHealthCheckMethod returns true if fullMethodName matches the standard gRPC health checking protocol ("/grpc.health.v1.Health/Check") or any variant containing "Health/Check". Used by both server and client handlers to suppress tracing for Kubernetes liveness/readiness probes.
...
// toSnakeLower converts a PascalCase or camelCase string to snake_case. It inserts an underscore before each uppercase letter that follows a non-uppercase, non-underscore character (e.g. "LoginUser" → "login_user", "GetHTTPStatus" → "get_httpstatus"). Used by grpcSpanNameFormatter to normalize method names.
...
// renamingClientHandler wraps otelgrpc's client stats handler to rename outbound RPC spans immediately after creation (in TagRPC) so they follow the "grpc.<snake_case_method>" naming convention in the trace backend. All other stats.Handler methods delegate directly to the wrapped base handler.
type renamingClientHandler struct {
	base stats.Handler
}

// newRenamingClientHandler creates a client stats handler that wraps otelgrpc's default client handler with span renaming. Health-check RPCs are filtered out.
...
// TagRPC delegates to the base handler (which creates the client span), then renames the span to the "grpc.<snake_case_method>" convention.
